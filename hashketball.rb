...
def player_numbers(team)
    if game_hash()[:home][:team_name] == team
      numbers = game_hash()[:home][:players].values().map do |player| 
        player[:number]
      end
    elsif game_hash()[:away][:team_name] == team
      numbers = game_hash()[:away][:players].values().map do |player| 
        player[:number]
      end
    end
    return numbers
...
def big_shoe_rebounds
  ...
  return game_hash()[team.to_sym][:players][player_biggest][:rebounds]
end
